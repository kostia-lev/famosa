...
if (isset($_POST['fpass'])) {
    ...
    $chk = $GLOBALS['db']->check1column("register", "email", $mail);
    if ($chk == 1) {
        $c = $GLOBALS['db']->singlerec("select * from register where email='$mail'");
        $usr = $c['email'];
        $pass = $c['password'];
        ...
        header("Location: $siteurl?fpmail");
        exit;
    } else {
        echo "<script>location.href='$siteurl?cmail';</script>";
        header("Location: $siteurl?cmail");
        exit;
    }
} else {
